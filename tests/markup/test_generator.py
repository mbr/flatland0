...
@pytest.fixture
def schema():
    return String.named(u'field1').using(default=u'val').from_defaults
...
@pytest.fixture(params=['jinja2', 'markupsafe', 'flatland.out.generic'])
...
def test_input_html(markup, expected, schema, el):
    ...
    got = generator.input(type=u'text', bind=el)
...
def test_textarea_explicit_contents(xmlgen, el):
    xmlgen.textarea.open(el, contents=u'xyzzy')
...
def test(xmlgen, el, markup_impl):
    ...
    assert xmlgen.label(contents=markup_impl(u'<x>')) == expected
